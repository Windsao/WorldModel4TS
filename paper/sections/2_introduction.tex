...
that do this well --- Moirai \citep{moirai}, Chronos \citep{chronos}, Time-MoE \citep{timemoe},
TimesFM \citep{timesfm} --- reach that ability by pretraining on time-series corpora of tens to
...
time. Video pretraining offers a different inheritance --- representations built from sequences, and
in the predictive variants an explicit forward model \citep{videomae, vjepa2} --- and the useful
question about it is not whether it raises the ceiling but whether it lowers the cost of reaching
one: \emph{does a video-pretrained backbone reach a given forecasting accuracy under a smaller
...
The result is a position on the accuracy--resource trade-off rather than a large accuracy jump.
Sixty thousand optimizer updates at an effective batch of 32 over 654M observations --- 18.9 hours on
one A40, about $0.3\%$ of the LOTSA archive and $1/27$ of the sample presentations behind VisionTS++
--- give one checkpoint that, under one inference rule, reaches a mean MSE of $0.285$ over six
...
it leads an ImageNet initialisation by $3$--$5\%$ at every horizon at 5k updates, and by 20k the
image initialisation has caught up; random initialisation of the same architecture is the worst arm
we ran, worse than an image-pretrained backbone $3.5\times$ smaller, so the effect is not capacity.
What video pretraining supplies here is adaptation efficiency --- the binding constraint in this
regime --- and not a higher ceiling.
...
\item Evidence that this reaches the accuracy band of the strongest published zero-shot models at
      ...
      (Sections~\ref{sec:main}--\ref{sec:budget}).
...
\item A diagnosis of why the naive route fails --- the backbone's own objective and forecasting
      accuracy move in opposite directions as pretraining continues --- which is the motivation for
      the framework and a caution for anyone continuing a vision objective on rendered series
