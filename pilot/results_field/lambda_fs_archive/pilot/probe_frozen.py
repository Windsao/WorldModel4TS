...
def ridge_skill(Fm, Y, lam=10.0, split=0.7):
    n = len(Fm); k = int(n * split)
    Xtr, Xte = Fm[:k], Fm[k:]; Ytr, Yte = Y[:k], Y[k:]
    mx = Xtr.mean(0, keepdims=True); sx = Xtr.std(0, keepdims=True) + 1e-8
    Xtr = (Xtr - mx) / sx; Xte = (Xte - mx) / sx
    my = Ytr.mean(0, keepdims=True); Ytr_c = Ytr - my
    d = Xtr.shape[1]
    W = np.linalg.solve(Xtr.T @ Xtr + lam * np.eye(d), Xtr.T @ Ytr_c)
    pred = Xte @ W + my
    probe_mse = float(np.mean((pred - Yte) ** 2))
    const_mse = float(np.mean((my - Yte) ** 2))          # predict train-mean
    return probe_mse, const_mse, probe_mse / (const_mse + 1e-12)
