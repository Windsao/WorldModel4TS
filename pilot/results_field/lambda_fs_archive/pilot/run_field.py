...
def run(model, Xtr, Ytr, Xte, Yte, epochs, lr, batch, mode, seed=0):
    ...
    model.eval()
    preds = []
    # eval batch: inference-only, so this has NO effect on the numbers (no batchnorm,
    # no cross-sample coupling) -- purely a memory/throughput knob. 256 spikes to ~22 GiB
    # per process on VideoMAE-base and OOMs as soon as a few runs share a GPU, which is
    # what killed the traffic run; 64 keeps the peak ~4x lower for the same results.
    eb = max(batch, 64)
    with torch.no_grad():
